...
public class ArrCharOps {
    ...
    */
    public static char[] concat(char[] arr1, char[] arr2) {
        char[] arraiesConcatenated = new char[arr1.length + arr2.length];
        for (int i = 0; i < arr1.length; i++) { 
            arraiesConcatenated[i] = arr1[i];
        }
        for (int i = 0; i < arr2.length; i++) { 
            arraiesConcatenated[i + arr1.length] = arr2[i];
        }
        return arraiesConcatenated;
    }
    ...
    public static char[] subArray(char[] arr, int beginIndex, int endIndex) {
        char[] subArrayOfOriginalArray = new char[endIndex - beginIndex]; 
        for (int i = beginIndex; i < endIndex; i++ ) {
            subArrayOfOriginalArray[i - beginIndex] = arr[i];
        }
        return subArrayOfOriginalArray;
    }
    ...
    public static int compareTo(String str1, String str2) {
        ...
        for (int i = 0; i < shorterStringLength; i++) {
            if (str1.charAt(i) != str2.charAt(i)) {
                resultOfComparison = (str1.charAt(i) < str2.charAt(i) ? -1 : 1);    // comparing the different character
